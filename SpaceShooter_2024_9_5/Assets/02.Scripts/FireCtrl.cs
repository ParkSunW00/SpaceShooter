...
//반드시 필요한 컴포넌트를 명시해 해당 컴포넌트가 삭제되는 것을 방지한다. Attribute
...
public class FireCtrl : MonoBehaviour
{
    ...
    //MuzzleFlash의 MeshRenderer 컴포넌트 변경 변수
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButton(0) && GameMgr.IsPointerOverUIObject() == false)
        {
            if (fireTimer <= 0.0f)
            { 
                Fire();
