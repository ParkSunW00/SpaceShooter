...
public class DragAndDropMgr : MonoBehaviour
{
    ...
    void MouseBtnUp()
    {
        ...
        { //아직 구매 할 수 없는 상태
            ...
            ShowMesssage("아직 이전 구매 처리 중입니다. 잠시 후 다시 시도해 주세요.");
            return;
        }
        ...
        for(int i = 0; i < m_InvenSlots.Length; i++)
        {
            if (IsCollSlot(m_InvenSlots[i]) == true)
            {
                ...
                {
                    //메시지 출력
                    ShowMesssage("해당 슬롯에는 아이템을 장착할 수 없습니다.");
                    continue;
                }

                if (BuySkItem(m_SaveIndex) == true)
                {  //여기서 상품 구매 시도 함수 호출(함수 호출 결과 성공일 때만 아래 코드 실행 되게)
                    a_BuyIndex = i;
                    break;
                }
            }//if (IsCollSlot(m_InvenSlots[i]) == true)
    ...
    {   //마우스가 UI 슬롯 위에 있는지? 판단하는 함수
        ...
        //마우스 좌표계 : 화면의 좌측하단 0, 0이고 우측상단(최고점 예 1280, 720)인 좌표계
    ...
    public void RecoverItem()   //이전 상태로 원복
    {
